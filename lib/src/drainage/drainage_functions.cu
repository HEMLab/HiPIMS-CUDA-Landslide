...
//--------global functions------------------------------------------------------------------------------------------------------
__global__ void pipe_boundaryKernal (int N, double *h_j_device, double *qx_j_device, double *qy_j_device,
...
__global__ void pipe_updateKernal(int CN, int num_Pipe, double a_speed, double dT, double *P_diameter_device, double *A_p_device, 
							...
							double *dx_cell_device, double *t_pipe_device, Const_variables constVariables);
...
__global__ void junction_calculationKernal(int N, int width_upsJP, int width_downsJP, double dt, double *junc_MN_device, 
									 ...
									 double *qyn_j_device, int *j_surfGird,
...
__device__ void pipeFrictionUpdate(double dt, double P_manning, double P_diameter, double F_n, double s2, 
...
void pipeGaugeOutput(double T, Drain_gauges &drainGauges, Pipe_variables pipeVariables){
	...
	for (int i=0;i<drainGauges.pGaugeNum;i++)
	{
		//fprintf(output, "     %f %f %f", drainGauges.pGaugeh[i], drainGauges.pGaugeQ[i], pipeVariables.A_p_out[drainGauges.pGaugePosi[i]]);
...
void readInputFile(char* Keywords[], Input_information &inputKeyPtr, 
	...
	// read the junction expansion file
	junctionExpansionFile(juncAttributes);

}

//----------------------------------------------------------------------------------------------------------------------
void junctionExpansionFile(Junc_attributes &juncAttributes){
	std::string line;
	std::ifstream InFile;
	InFile.open("input/drainage/junctionExpansion.dat");
	if (!InFile)
		std::cout << "No junctionExoansion file exists" << std::endl;
	else
	{
		int n = 1;  // ptr of the row when reading the file
		int num_ptr = 0;  // ptr if the row of the vector data
		...
		while (getline(InFile, line))
		{
			...
			{
				strm >> node_int;
				juncAttributes.J_surfExGrid[num_ptr] = node_int;
				num_ptr++;
			}
	...
	while (getline(InFile,line))
	{
		...
		if (token[0]=='[')
		{
			...
			switch (newsect)
			{
				...
				case 4:
					inputKeyPtr.p_juncExpanValue=n;
					//std::cout<<"row of junction expansion value= "<< inputKeyPtr.p_juncExpanValue <<std::endl;
					break;
...
void secondOpenIntroductionFile(Input_information &inputKeyPtr){
	...
	while (getline(InFile,line))
	{
		...
		else if (n == (inputKeyPtr.p_juncExpanValue + 1))
		{
			strm >> inputKeyPtr.junc_expanValue;
			//std::cout << "junction expansion value= " << inputKeyPtr.junc_expanValue << std::endl;
		}
...
void hostMemoryCreationForObjects(Input_information &inputKeyPtr, 
	...
	juncAttributes.setHostMemory(inputKeyPtr.num_Junc, inputKeyPtr.junc_expanValue);
...
void allocMemory(Input_information inputKeyPtr,
	...
	juncVariables.setDeviceMemory(inputKeyPtr.num_Junc, inputKeyPtr.junc_expanValue);
...
void copyMemory(Input_information inputKeyPtr,
	...
	juncVariables.copyCudaMemory(inputKeyPtr.num_Junc, juncAttributes.J_elev, 
		juncAttributes.J_manning, juncAttributes.J_surfGrid, juncAttributes.J_surfExGrid, 
		inputKeyPtr.junc_expanValue,juncAttributes.J_radius,
...
__global__ void pipe_boundaryKernal (int N, double *h_j_device, double *qx_j_device, double *qy_j_device,
	...
	while (tid< N)
	{
		...
		else
		{
		 ...
		 // if (tid<10)
		 // {
		 // 	 printf("upNorm1_device[%d]= %f  \n", tid, upNorm1_device[tid]);
   //      	 printf("upNorm2_device[%d]= %f \n",  tid, upNorm2_device[tid]);
		 // 	 printf("upsPJ_device[%d]= %d  \n",tid, upsPJ_device[tid]);
			//  printf("downsPJ_device[%d]= %d  \n",tid, downsPJ_device[tid]);
			//  printf("downsPO_device[%d]= %d  \n",tid, downsPO_device[tid]);
			//  printf("p_ups_h_device[%d]= %f  \n",tid, p_ups_h_device[tid]);
			//  printf("p_downs_h_device[%d]= %f  \n",tid, p_downs_h_device[tid]);
			//  printf("p_ups_Q_device[%d]= %f  \n",tid, p_ups_Q_device[tid]);
			//  printf("p_downs_Q_device[%d]= %f  \n",tid, p_downs_Q_device[tid]);
...
//	double mass_pipe=0;  // to verify mass conservation
	...
	/*		printf("pipe_flux_downs1[%d]= %f\n",tid, pipe_flux_downs1[j]);
	 ...
	 //   		if (tid==17)	
		...
		pipeFrictionUpdate(dt, pipe_MN_device[j], P_diameter_device[j], dqxdt, s2,  A_p_device[tid], Q_p_device[tid],
	...
	int n=interval_ptr_device;
		  ...
		  else
		  {
			  ...
			  if (T>=timeSeries[timeStart+n])
			  {
				  ...
				  for (int i=timeStart+n+1; i<=timeStart+n+seriesLength; i++)
				  {
		   ...
		   //    printf("**timeSeries[%d] = %f    ", timeStart+n, timeSeries[timeStart+n]);
...
__device__ void pipeFrictionUpdate(double dt, double P_manning, double P_diameter, double F_n, double s2, 
	...
	//printf("B= %f, P_manning= %f, Pn= %f, SGN= %f\n",B, P_manning, P_n, SGN );
